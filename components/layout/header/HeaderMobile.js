...
const HeaderMobile = () => {
  ...
  return (
    <header
      className="fixed top-0 right-0 xl:w-4/12 lg:w-4/12 w-full h-screen hidden animate-slide-out-menu border-b-8 border-t-8 border-secondary"
      id="mobileMenu"
    >
      <div className="relative flex flex-col items-center justify-around h-full bg-primary text-white">
        <div role="menu" className="flex flex-col items-center">
          {links.map((item, idx) => (
            <Link
              ...
              href={item.link}
              key={idx}
              className={`md:text-2xl text-xl border-b-4 my-4 ${
                pathName === item.link
        ...
        <a href="#" role="none" onClick={closeMenu} className="">
          <IconBtn
            iconName={cgIcon.CgClose}
            btnColor="white"
            aria-label="Fermer le menu"
            iconColor="secondary"
            addClass="ring-4 ring-white focus:ring-white ring-opacity-50"
          />
        </a>
      </div>
    </header>
